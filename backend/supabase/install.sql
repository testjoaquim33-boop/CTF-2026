-- ####################  PROJECT_FIT INSTALL V2 (ARRAY SYNTAX)  ####################
-- Si tu vois cette ligne 'INSTALL V2' dans l'éditeur, c'est la BONNE version.
-- Reset + install complet, en une transaction. Destructif sur le schéma public
-- (projet neuf uniquement). N'affecte pas les schémas Supabase (auth, storage).
-- #################################################################################
...
-- Sentinel de fin : compte les exercices chargés.
do $sentinel$ begin raise notice 'PROJECT_FIT install OK — exercises=%', (select count(*) from exercises); end $sentinel$;
