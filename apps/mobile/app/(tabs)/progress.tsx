...
export default function ProgressScreen() {
  ...
  return (
    <SafeAreaView style={{ flex: 1, backgroundColor: t.colors.bg }}>
      ...
      >
        <Text variant="h1">{tr('pg.title')}</Text>

        <View style={{ flexDirection: 'row', gap: t.spacing.md }}>
          <StatTile label={tr('pg.workouts')} value={String(data?.workoutCount ?? 0)} />
          <StatTile label={tr('pg.currentWeight')} value={data?.currentWeightKg ? `${data.currentWeightKg} kg` : '—'} />
          <StatTile label={tr('pg.target')} value={data?.targetWeightKg ? `${data.targetWeightKg} kg` : '—'} />
        </View>

        <View style={{ backgroundColor: t.colors.bgElevated, borderRadius: t.radius.md, padding: t.spacing.lg,
          borderWidth: 1, borderColor: t.colors.border }}>
          <Text variant="h3" style={{ marginBottom: t.spacing.md }}>{tr('pg.bodyweight')}</Text>
          <MiniBarChart values={(data?.bodySeries ?? []).map((b) => b.weightKg)} />
        </View>

        <View>
          <Text variant="h3" style={{ marginBottom: t.spacing.md }}>{tr('pg.recentRecords')}</Text>
          {(data?.recentPRs ?? []).length === 0 ? (
            <Text color="textMuted">{tr('pg.noRecords')}</Text>
          ) : (
            data!.recentPRs.map((pr, i) => (
              <View key={i} style={{ flexDirection: 'row', justifyContent: 'space-between',
                ...
                <View>
                  ...
                  <Text variant="caption" color="textSecondary">{tr(`pr.${pr.type}`) !== `pr.${pr.type}` ? tr(`pr.${pr.type}`) : pr.type}</Text>
                </View>
